/* =============================================================================
   EEC Géolocalisation — Icon set + marker generators
   Shared module: registers globals via Object.assign(window, ...)
   ============================================================================= */
...
const PATHS = {
  ...
  trail:       <><circle cx="6" cy="6" r="1.5"/><circle cx="12" cy="9" r="1.5"/><circle cx="18" cy="13" r="1.5"/><circle cx="14" cy="18" r="1.5"/><path d="M6 6l6 3 6 4-4 5" stroke-dasharray="2,3"/></>,
...
const Icon = ({ name, size = 18, color = 'currentColor', stroke = 1.7, fill = 'none' }) => {
  const p = PATHS[name];
  if (!p) return null;
  return (
...
function markerSvg(type, color, sel) {
...
function makeIcon(type, color, sel = false) {
  const sz = sel ? [36, 44] : [30, 38];
  return L.divIcon({
...
Object.assign(window, { PATHS, Icon, TYPE_ICON, markerSvg, makeIcon });
