/* =============================================================================
   EEC Géolocalisation — Mock data generator
   22 régions synodales · 137 districts · 576 paroisses · 311 œuvres · 685 ouvriers
   ============================================================================= */

// 22 régions synodales — villes principales du Cameroun (coords réelles, polygones approximés)
const REGIONS = [
...
// District naming pools (quartiers + communes communs au Cameroun)
const DISTRICT_SUFFIXES = [
  'Centre', 'Nord', 'Sud', 'Est', 'Ouest', 'Akwa', 'Bonabéri', 'Bonapriso', 'Bonanjo',
  'Mfoundi', 'Mvog-Mbi', 'Ngoa-Ekellé', 'Nlongkak', 'Bastos', 'Madagascar',
  'Marché', 'Plateau', 'Quartier Haut', 'Quartier Bas', 'Tongo', 'Banengo',
  'Tsinga', 'Briqueterie', 'Mokolo', 'Mendong', 'Etoa-Meki', 'Nkolbisson',
  'Deido', 'New Bell', 'Bépanda', 'Logbessou', 'Logpom', 'PK10', 'PK14',
  'Up Station', 'Down Town', 'Mile 17', 'Buea Town', 'Molyko', 'Soppo',
  'Banja', 'Bafou', 'Foreké', 'Bandjoun', 'Baham', 'Bayangam', 'Bandenkop',
];

// Pools de noms / prénoms camerounais (pasteurs)
...
const SURNAMES = ['Ekanga', 'Ateba', 'Biya', 'Mvondo', 'Eyenga', 'Owona', 'Mballa', 'Essomba', 'Ndongo', 'Nkomo', 'Tchoumi', 'Kamdem', 'Nguefack', 'Fotso', 'Tchaptchet', 'Sop', 'Wouassi', 'Tsafack', 'Djamou', 'Mbarga', 'Etoundi', 'Ndoumbé', 'Mounjouenpou', 'Nyobe', 'Bahoken', 'Ngongang', 'Kemajou', 'Ndi', 'Tabi', 'Bongmba'];
...
// Types d'entités cartographiées
const ENTITY_TYPES = [
  ...
  { id: 'univ',     label: 'Universités / Inst. supérieurs', singular: 'Université', color: '#4A148C', target: 12 },
...
const jitter = (m, amp) => m + (rand() - 0.5) * 2 * amp;

// Generate districts per region
  ...
  REGIONS.forEach(r => {
    ...
    for (let i = 0; i < r.districts; i++) {
      ...
      districts.push({
        ...
        // small offset around region center
...
// Generate parishes distributed across districts so total ≈ 576
function buildParishes(districts) {
  const out = [];
  // Each district gets 3-6 parishes weighted by region population
  const TARGET = 576;
  ...
  districts.forEach(d => {
    ...
    for (let i = 0; i < n && pId <= TARGET; i++) {
      ...
      const baptized = Math.floor(fid * 0.85);
      ...
      out.push({
        ...
        regionId: d.regionId,
        districtId: d.id,
        name: parishName(pId, region.city, d.name),
        lat: jitter(d.lat, 0.08),
        lng: jitter(d.lng, 0.08),
        stats: {
          fideles: fid,
          communiants: commun,
          nonCommuniants: fid - commun,
          baptemes: Math.floor(8 + rand() * 30),
          mariages: Math.floor(2 + rand() * 12),
          deces: Math.floor(rand() * 6),
        },
        photo: pId % 7 === 0,
      });
      pId++;
...
function parishName(idx, city, districtName) {
  // mix of named-after-city + by-village
...
// Generate œuvres distributed across regions
  ...
  ENTITY_TYPES.filter(t => t.id !== 'paroisse').forEach(type => {
    for (let i = 0; i < type.target; i++) {
      ...
      out.push({
        ...
        regionId: r.id,
        name,
        lat: jitter(r.lat, r.radius * 0.6),
        lng: jitter(r.lng, r.radius * 0.6),
        capacity: Math.floor(40 + rand() * 600),
        year: 1950 + Math.floor(rand() * 73),
      });
    }
  });
  return out;
...
function buildWorkers(parishes) {
  const out = [];
  let id = 1;
  // Total 685 — distribute across parishes
  // First place 1 evêque per region
  REGIONS.forEach(r => {
    const surname = pick(SURNAMES);
    const first = pick(NAMES_M);
    out.push({
...
const PARISHES = buildParishes(DISTRICTS);
const OEUVRES = buildOeuvres();
const WORKERS = buildWorkers(PARISHES);

// counts
const ENTITY_COUNTS = ENTITY_TYPES.reduce((acc, t) => {
  acc[t.id] = t.id === 'paroisse'
    ? PARISHES.length
    : OEUVRES.filter(o => o.type === t.id).length;
  return acc;
}, {});

// Combined items list for map / search
const ALL_ITEMS = [...PARISHES, ...OEUVRES];

window.EEC_DATA = {
  REGIONS, DISTRICTS, PARISHES, OEUVRES, WORKERS,
  ENTITY_TYPES, ENTITY_COUNTS, GRADES,
  ALL_ITEMS,
};
