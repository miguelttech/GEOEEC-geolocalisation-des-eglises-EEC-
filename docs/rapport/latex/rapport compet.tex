...
\noindent
L'Église Évangélique du Cameroun regroupe 565~paroisses, 311~œuvres sociales
et éducatives, ainsi que 685~ouvriers ecclésiastiques répartis dans 22~régions
synodales et 137~districts couvrant l'ensemble du territoire national, sans
...
\noindent
The Evangelical Church of Cameroon encompasses 565~parishes, 311~social and
...
\begin{figure}[H]
  ...
  \includegraphics[width=1.8\textwidth]{diagrammes-Diagramme de Contexte.png}
...
Le diagramme d'activité  modélise le flux de
...
Django~5~[16] a été retenu comme framework backend principal
...
GeoServer~2.26~[17] a été retenu pour la publication des
...
Next.js~14~[18] avec l'App Router a été retenu pour le frontend
...
Leaflet.js~[19] a été retenu pour la bibliothèque
...
Redis~7~[20] assure deux fonctions complémentaires~:
...
\begin{figure}[H]
  \center
  \includegraphics[width=1\textwidth]{deploiement.png}
   \captionInterface{ publique Carte interactive 
   Compose}
  \label{fig:diag-contexte}
\end{figure}
...
\begin{figure}[H]
  ...
  \captionInterface{  carte vu sous le detail
Compose}
  \label{fig:diag-contexte}
\end{figure}
...
begin{figure}[H]
  ...
  \captionInterface{ Interface d’administration — Tableau de bord de l’administrateur national}
  \label{fig:diag-contexte}
\end{figure}
...
begin{figure}[H]
  ...
  \captionInterface{ nterface d’administration — Gestion des paroisses avec contrôle RBA}
  \label{fig:diag-contexte}
\end{figure}
...
begin{figure}[H]
  ...
  \captionInterface{ Module d’import — Chargement de données Excel et rapport de validation}
  \label{fig:diag-contexte}
\end{figure}
...
begin{figure}[H]
  ...
  \captionInterface{ Journal d’audit — Historique des opérations d’écriture sur les données}
  \label{fig:diag-contexte}
\end{figure}
...
en base mais ne s'affichent pas sur la carte. La plateforme ne peut donc
...
La fonction de calcul d'itinéraire utilise la formule de Haversine, qui
mesure la distance géodésique entre deux points en ligne droite. Cette
mesure surestime systématiquement la durée réelle de trajet sur un réseau
routier camerounais caractérisé par des voies sinueuses et un relief
accidenté~; elle ne tient pas compte des barrières physiques (fleuves,
massifs montagneux). La durée estimée à 60~km/h de moyenne est donc
approximative et ne peut servir de base à des décisions opérationnelles
précises.
...
coordonnées GPS des 127~paroisses non géolocalisées reste manuelle et
...
plateforme, puis exposé les résultats à travers sept interfaces fonctionnelles
commentées au regard des exigences du Chapitre~2. La discussion critique a
...
127~paroisses sur 565 (22~\%) ne disposent d'aucune coordonnée GPS valide
dans les fichiers sources de l'EEC~: la plateforme les importe mais ne peut
pas les afficher sur la carte, ce qui réduit d'autant sa capacité à soutenir
des décisions de planification territoriale fondées sur la couverture
cartographique réelle. Deuxièmement, la fonction de calcul d'itinéraire
repose sur la formule de Haversine, qui mesure une distance en ligne droite
et ne tient pas compte du réseau routier camerounais réel, induisant des
estimations de durée de trajet systématiquement sous-évaluées pour les
zones à relief accidenté ou à voies sinueuses. Troisièmement, l'interface
...
terrain des coordonnées GPS manquantes reste donc sans outillage dédié.
...
\begin{longtable}{%
  >{\bfseries\color{eccdarkgreen}\centering\arraybackslash}m{1.6cm}
  m{11.4cm}}
\caption{Références bibliographiques du rapport}\label{tab:references} \\
\toprule
\rowcolor{eecgreen}
\multicolumn{1}{>{\bfseries\color{white}\centering\arraybackslash}m{1.6cm}}{Numéro} &
\multicolumn{1}{>{\bfseries\color{white}}m{11.4cm}}{Référence bibliographique} \\
\midrule
\endfirsthead
...
[16] & Django Software Foundation, \textit{Django Documentation}, version~5.0, 2024. Disponible sur~: \url{https://docs.djangoproject.com/en/5.0/} \\
\rowcolor{eeclightgreen!60}
[17] & GeoServer Development Team, \textit{GeoServer 2.26 User Manual}, 2024. Disponible sur~: \url{https://docs.geoserver.org/stable/en/user/} \\
[18] & Vercel, \textit{Next.js Documentation}, version~14, 2024. Disponible sur~: \url{https://nextjs.org/docs} \\
\rowcolor{eeclightgreen!60}
[19] & V.~Agafonkin, \textit{Leaflet: An open-source JavaScript library for mobile-friendly interactive maps}, 2024. Disponible sur~: \url{https://leafletjs.com} \\
[20] & Redis Ltd., \textit{Redis Documentation}, 2024. Disponible sur~: \url{https://redis.io/docs} \\
\end{longtable}
