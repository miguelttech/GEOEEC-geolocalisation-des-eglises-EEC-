...
L'algorithme de \textbf{Dijkstra} (1956) résout le problème~\eqref{eq:plus-court-chemin}
pour un graphe à poids positifs. Il maintient, pour chaque sommet $v$, une
...
L'algorithme \textbf{A\*} (A-star) améliore Dijkstra en orientant la
...
Next.js~16~[18] avec l'App Router a été retenu pour le frontend
...
\textbf{OpenStreetMap}~:
\begin{itemize}[itemsep=2pt, topsep=4pt, leftmargin=1.5cm]
  \item \textbf{OSRM} (\textit{Open Source Routing Machine}) --- instance
  ...
  \item \textbf{Valhalla} (instance publique FOSSGIS) est sollicité en
...
préliminaire~: 480~paroisses sources sans coordonnées GPS valides ont ainsi
...
Elle repose sur \textbf{MapLibre~GL~JS} (rendu WebGL), qui permet une
...
\subsubsection{Nouvelles bibliothèques intégrées}

Ces fonctionnalités ont nécessité l'ajout de deux dépendances au frontend,
récapitulées dans le tableau~\ref{tab:outils}~:
\texttt{maplibre-gl} pour le rendu cartographique~3D par WebGL, et
l'utilisation des services publics \textbf{OSRM} et \textbf{Valhalla} côté
backend (aucune bibliothèque supplémentaire n'étant requise, ces services
étant consommés par simples requêtes HTTP dans le module
\texttt{routing.py}). Ce choix préserve la légèreté de la pile logicielle~:
la carte 2D principale continue de reposer sur Leaflet~1.9 (42~Ko minifié),
tandis que MapLibre~GL n'est chargé --- à la demande --- que pour la durée
effective d'une navigation active.
