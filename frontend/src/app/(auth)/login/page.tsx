...
export default function LoginPage() {
  ...
  // Le reste de l'app met body { overflow: hidden } pour la carte — on le reset ici
  useEffect(() => {
    document.body.style.overflow = 'auto';
    return () => { document.body.style.overflow = ''; };
  }, []);

  // Récupère le token CSRF (nécessaire pour les requêtes POST Django)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.location.href = data.force_password_change
        ...
        <div className={`ls-box${shake ? ' ls-shake' : ''}`}>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="ls-opts">
              ...
              <Link href="/auth/mot-de-passe-oublie" className="ls-forgot">
                Mot de passe oublié ?
              </Link>
            </div>
          ...
          <div className="ls-sep"><span>ou</span></div>

          <button type="button" className="ls-sso">
            <IconGlobe />
            Connexion via SSO synodal
          </button>
...
/* ── Icônes (composants purs, pas de state) ── */
