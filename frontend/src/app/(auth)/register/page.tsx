...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${BACKEND}/api/visitor/register/`, {
        method: 'POST',
        credentials: 'include',
          ...
          {success ? (
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="ls-field">
                ...
                <div className="ls-iw">
                  <IconMail />
                  <input id="rs-email" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="j.ateba@exemple.cm" required autoComplete="email" />
                </div>
              </div>

              <div className="ls-field">
                <label htmlFor="rs-pw">Mot de passe <span style={{ color: 'var(--m)', fontWeight: 400 }}>(8 car. min.)</span></label>
                <div className="ls-iw">
                  <IconLock />
                  <input id="rs-pw" type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••••" required autoComplete="new-password" />
              ...
              <button type="submit" disabled={loading || !email || !password || !confirm} className="ls-submit">
                {loading ? <span className="ls-spinner" /> : <><span>Créer mon compte</span> <IconArrow /></>}
              </button>
            </form>
          )}
...
function PasswordStrength({ password }: { password: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
      {[0,1,2,3].map(i => (
        <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i < score ? colors[score] : 'rgba(240,244,241,0.12)', transition: 'background 0.3s' }} />
      ))}
