  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${BACKEND}/api/auth/change-password/`, {
        ...
        body: JSON.stringify({ old_password: current, new_password: password, confirm_password: confirm }),
      });
    ...
    <div className="ls-root">
      ...
      <div className="ls-right">
        <div className={`ls-box${shake ? ' ls-shake' : ''}`}>
          ...
          <h1 className="ls-h1">Changer votre <em>mot de passe.</em></h1>
          <p className="ls-sub">
            Pour votre sécurité, vous devez définir un nouveau mot de passe
            avant d&apos;accéder à votre espace de travail.
          </p>
          ...
          {success && (
            <div className="ls-success" role="status" style={{ marginBottom: 16 }}>
              <IconCheck />
              <div>
                <strong>Mot de passe mis à jour !</strong>
                <div style={{ marginTop: 4, fontSize: 12 }}>Redirection vers votre espace de travail…</div>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} noValidate style={{ display: success ? 'none' : 'block' }}>
            ...
            <div className="ls-field">
              ...
              <div className="ls-iw">
                <IconLock />
                <input id="cp-cur" type="password" value={current} onChange={e => setCurrent(e.target.value)} placeholder="••••••••••" required autoComplete="current-password" />
              </div>
            </div>

            <div className="ls-field">
              <label htmlFor="cp-pw">Nouveau mot de passe <span style={{ color: 'var(--m)', fontWeight: 400 }}>(12 car. min.)</span></label>
              <div className="ls-iw">
                <IconLock />
                <input id="cp-pw" type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••••" required autoComplete="new-password" />
            ...
            <div className="ls-field">
              <label htmlFor="cp-cf">Confirmer le nouveau mot de passe</label>
              <div className="ls-iw">
                <IconLock />
                <input
                  id="cp-cf"
            ...
            <button type="submit" disabled={loading || !current || !password || !confirm} className="ls-submit">
              {loading ? <span className="ls-spinner" /> : <><span>Valider le nouveau mot de passe</span> <IconArrow /></>}
            </button>
          </form>
        </div>
      </div>
