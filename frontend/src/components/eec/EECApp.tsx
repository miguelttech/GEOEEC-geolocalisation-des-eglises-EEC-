...
/* ============================================================
   Types
   ============================================================ */
type AnyItem = typeof ALL_ITEMS[number];
  ...
  useEffect(() => {
    ...
    filteredItems.forEach(it => {
      ...
      const m = L.marker([it.lat, it.lng], {
        icon: makeIcon(it.type, color, it.id === selectedId),
        title: it.name,
      });
          ...
          <>
            ...
            <div className="panel-section">
              ...
              {recents.length === 0 ? (
                ...
                return (
                  <button key={it.id + (it as any)._at} className="list-row" onClick={() => onPick('item', it)} style={{padding:'8px 0', borderBottom:'1px solid var(--border-soft)'}}>
...
const DetailPanel = ({ item, onClose, saved, onToggleSave }: {
  ...
  const district = (item as any).districtId ? DISTRICTS.find(d => d.id === (item as any).districtId) : null;
  const isParish = item.type === 'paroisse';
  const typeMeta = ENTITY_TYPES.find(t => t.id === item.type);
    ...
    <div className="detail-panel open">
        ...
        <div className="hero-fallback">
          ...
          <div style={{position:'absolute', left:'50%', top:'50%', transform:'translate(-50%, -55%)', display:'flex', alignItems:'center', justifyContent:'center'}}>
            <div style={{width:76, height:76, borderRadius:'50%', background: (typeMeta?.color ?? '#2E9744') + '1E', border:`2px solid ${typeMeta?.color ?? '#2E9744'}38`, display:'flex', alignItems:'center', justifyContent:'center'}}>
              <Icon name={TYPE_ICON[item.type]} size={34} color={typeMeta?.color} stroke={1.6} />
            </div>
          </div>
      ...
      <div className="dp-head">
        ...
        <div className="dp-meta-line">
          ...
          <span>{district ? district.name : region?.city}</span>
          <span className="ml-sep">·</span>
      ...
      {tab === 'apercu' && isParish && (item as any).stats && (
        <>
          <div className="dp-section">
            ...
            <div className="big-stat-row">
              <div className="big-stat"><div className="bs-value" style={{color:'var(--t-1)'}}>{(item as any).stats.baptemes}</div><div className="bs-label">Baptêmes</div></div>
              <div className="big-stat"><div className="bs-value" style={{color:'var(--t-1)'}}>{(item as any).stats.mariages}</div><div className="bs-label">Mariages</div></div>
              <div className="big-stat"><div className="bs-value" style={{color:'var(--t-1)'}}>{(item as any).stats.deces}</div><div className="bs-label">Décès</div></div>
            </div>
          </div>
          {workers.length > 0 && (
            <div className="dp-section">
          ...
          <div className="dp-foot-cards">
            ...
            <button className="dp-foot-card">
              <span className="fc-ico"><Icon name="directions" size={16} stroke={2} /></span>
        ...
        <>
          <div className="dp-section">
            ...
            <div className="big-stat-row">
              ...
              <div className="big-stat orange"><div className="bs-value">{2025 - ((item as any).year ?? 2000)}</div><div className="bs-label">Ans d'activité</div></div>
            </div>
          </div>
          <div className="dp-foot-cards">
      ...
      {tab === 'infos' && (
        <div className="dp-section">
          ...
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
            ...
            <InfoLine ico="clock" label={isParish ? 'Culte dominical : 09 h 00 · 11 h 00' : `Fondée en ${(item as any).year}`} sub={isParish ? 'Étude biblique : mercredi 18 h 00' : `Capacité : ${(item as any).capacity} places`} />
      ...
      {tab === 'stats' && isParish && (item as any).stats && (
        <div className="dp-section">
          ...
          <div className="big-stat-row">
            ...
            <div className="big-stat red"><div className="bs-value">{(item as any).stats.deces}</div><div className="bs-label">Décès</div></div>
          </div>
        </div>
      )}
      ...
      {items.slice(0, 200).map(it => {
        ...
        return (
          <div key={it.id} className="lv-row" onClick={() => onPick(it)}>
            ...
            <span className="lv-num">{(it as any).stats ? (it as any).stats.fideles : '—'}</span>
...
export default function EECApp() {
  ...
  const filteredItems = useMemo(() => ALL_ITEMS.filter(it => {
    if (!filters.layers[it.type]) return false;
    if (filters.region && it.regionId !== filters.region) return false;
    if (filters.district && (it as any).districtId !== filters.district) return false;
    if (filters.parish && it.id !== filters.parish) return false;
    if (it.type === 'paroisse' && (it as any).stats) {
      if (filters.minFideles && (it as any).stats.fideles < filters.minFideles) return false;
      if (filters.minCommun && (it as any).stats.communiants < filters.minCommun) return false;
    }
    return true;
  }), [filters]);
      ...
      <div className={'main' + (activeTab ? ' panel-open' : '') + (railCollapsed ? ' rail-collapsed' : '') + (fullscreen ? ' fullscreen' : '')}>
        ...
        {!fullscreen && (
          <aside className="panel">
            ...
            {activeTab === 'paroisses' && <EntityListPanel title="Paroisses" items={PARISHES} kind="parish" onPick={handleMarkerClick} onClose={() => setActiveTab(null)} totalLabel={`${PARISHES.length} paroisses recensées`} />}
            {activeTab === 'oeuvres'   && <EntityListPanel title="Œuvres" items={OEUVRES} kind="oeuvre" onPick={handleMarkerClick} onClose={() => setActiveTab(null)} totalLabel={`${OEUVRES.length} œuvres recensées`} />}
