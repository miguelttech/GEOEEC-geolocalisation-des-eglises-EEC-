...
// Tabs locked in public mode (require login)
const LOCKED_FOR_PUBLIC = new Set(['parcours', 'history', 'ouvriers']);

/* ============================================================
   useLeafletMap hook (identical to EECApp)
   ============================================================ */
function useLeafletMap(
  ...
  useEffect(() => {
    ...
    const map = L.map(mapId, {
      center: CAMEROON_CENTER, zoom: 6, minZoom: 5, maxZoom: 18,
    ...
    REGIONS.forEach(r => {
      const circle = L.circle([r.lat, r.lng], {
        radius: r.radius * 110000, color: '#2E9744', weight: 1.6, opacity: 0.7,
        fillColor: '#2E9744', fillOpacity: 0.06, interactive: true,
      });
      const pc = PARISHES.filter(p => p.regionId === r.id).length;
      const dc = DISTRICTS.filter(d => d.regionId === r.id).length;
      circle.bindPopup(
        `<div class="region-popup"><div class="rp-strip"></div><div class="rp-type">Région Synodale</div><div class="rp-name">${r.city}</div><div class="rp-stats"><span>Districts</span><b>${dc}</b><span>Paroisses</span><b>${pc}</b></div></div>`,
  ...
  useEffect(() => {
    ...
    filteredItems.forEach(it => {
      ...
      const m = L.marker([it.lat, it.lng], { icon: makeIcon(type, t?.color ?? '#2E9744', it.id === selectedId), title: it.name });
...
const MapNavbar = ({ view, setView, stats, theme, setTheme, user, mode, onLogout }: {
  ...
  stats: Record<string, number>; theme: string; setTheme: (t: string) => void;
  ...
  return (
    <header className="navbar">
      ...
      <div className="nav-actions">
        ...
        {user ? (
          <>
            <button className="admin-pill" title={user.email}>
            ...
            <button className="icon-btn" onClick={onLogout} title="Se déconnecter" style={{ color: 'var(--t-2)' }}>
...
/* ============================================================
   MODE-AWARE RAIL
   ============================================================ */
const MapRail = ({ active, setActive, onCollapse, badges, mode, onLockedClick }: {
  active: string | null; setActive: (id: string | null) => void;
  onCollapse: () => void; badges: Record<string, number | null>;
  ...
  <nav className="rail">
    {ALL_RAIL_TABS.map(t => {
      ...
      return (
        <button key={t.id}
          className={'rail-tab' + (active === t.id ? ' active' : '') + (locked ? ' locked' : '')}
          onClick={() => locked ? onLockedClick() : setActive(active === t.id ? null : t.id)}
          title={locked ? `${t.label} — Connexion requise` : t.label}
          style={locked ? { opacity: 0.38, cursor: 'not-allowed' } : undefined}
        >
          <Icon name={t.icon} size={20} stroke={1.7} />
          <span className="lbl">{t.label}</span>
          {locked && (
            <span className="badge" style={{ background: 'var(--t-3)', fontSize: 8 }}>
              <Icon name="lock" size={9} stroke={2} />
            </span>
          )}
          {!locked && badges[t.id] != null && <span className="badge">{badges[t.id]}</span>}
        </button>
      );
    })}
...
/* ============================================================
   LOGIN PROMPT (modal for locked features)
   ============================================================ */
...
/* ============================================================
   AUTH CTA BANNER (bottom banner for public mode)
   ============================================================ */
...
const FiltersPanel = ({ filters, setFilters, layerCounts, onClose, onReset, onApply }: {
  ...
  const districtOptions: SelectOption[] = [{ value: '', label: filters.region ? `Tous les districts (${districts.length})` : '— Choisir une région d\'abord' }, ...districts.map(d => ({ value: d.id, label: d.name.replace('District de ', '') }))];
  const parishOptions: SelectOption[] = [{ value: '', label: filters.district ? `Toutes les paroisses (${parishes.length})` : '— Choisir un district d\'abord' }, ...parishes.map(p => ({ value: p.id, label: p.name }))];
...
const StatsPanel = ({ filters, layerCounts, onClose, onFocusRegion }: {
  filters: FilterState; layerCounts: Record<string, number>; onClose: () => void; onFocusRegion: (r: Region) => void;
}) => {
  ...
  const regionStats  = useMemo(() => {
    ...
    return REGIONS.map(r => { const count = PARISHES.filter(p => p.regionId === r.id).length; return { ...r, count, pct: (count / max) * 100 }; }).sort((a, b) => b.count - a.count);
  }, []);
  return (
    <>
      ...
      <div className="panel-body">
        <div className="panel-section">
          ...
          <div className="stats-row">
            <div className="stat-card featured"><div className="sc-label">Total fidèles</div><div className="sc-value">{fmt(totalFideles)}</div><div className="sc-delta">+ 3,2 % vs 2024</div></div>
            <div className="stat-card yellow"><div className="sc-label">Communiants</div><div className="sc-value">{fmt(totalCommun)}</div><div className="sc-delta">{Math.round(totalCommun / totalFideles * 100)} % du total</div></div>
          </div>
        </div>
        <div className="panel-section">
          <h3 className="section-label">Top régions</h3>
...
/* Entity List Panel (no edit buttons for visitors) */
...
/* Parcours Panel */
const ParcoursPanel = ({ onClose, mode }: { onClose: () => void; mode: MapMode }) => (
  <>
    <div className="panel-head"><h2>Parcours &amp; Itinéraires</h2><button className="ib" onClick={onClose}><Icon name="close" size={16} stroke={2} /></button></div>
    <div className="panel-body">
      <div className="panel-section">
        <h3 className="section-label">Calculer un itinéraire</h3>
        <div className="loc-row" style={{ marginBottom: 8 }}>
          <span className="loc-glyph" style={{ background: 'var(--eec-green)', color: '#fff' }}><Icon name="pinFilled" size={13} fill="#fff" stroke={0} /></span>
          <div className="loc-body"><span className="loc-label">Point de départ</span><div style={{ fontSize: 12, color: 'var(--t-2)', marginTop: 2 }}>Position actuelle</div></div>
        </div>
        <div className="loc-row">
          <span className="loc-glyph" style={{ background: 'var(--eec-yellow)', color: '#1A1A1A' }}><Icon name="flag" size={13} fill="#1A1A1A" stroke={0} /></span>
          <div className="loc-body"><span className="loc-label">Destination</span><div style={{ fontSize: 12, color: 'var(--t-3)', marginTop: 2 }}>Sélectionnez une paroisse</div></div>
        </div>
        <button className="btn-apply" style={{ marginTop: 10, width: '100%' }}><Icon name="route" size={14} stroke={2} /> Calculer</button>
      </div>
      <div className="panel-section">
        ...
        {[
          ...
          <button key={i} className="list-row" style={{ padding: '10px 0', borderBottom: '1px solid var(--border-soft)' }}>
            ...
            <span className="lr-body"><span className="lr-title">{p.name}</span><span className="lr-sub">{p.d}</span><span className="lr-sub" style={{ color: 'var(--green-deep-text)', fontWeight: 600 }}>{p.km}</span></span>
            <span className="lr-chev"><Icon name="chevron" size={14} stroke={2} /></span>
          </button>
        ))}
      </div>
      {mode === 'visitor' && (
        <div className="panel-section">
          <h3 className="section-label">Mes itinéraires sauvegardés</h3>
          <div className="search-empty" style={{ padding: '20px 0' }}>
            <div className="em-glyph"><Icon name="trail" size={22} stroke={1.6} /></div>
...
/* Settings Panel */
const SettingsPanel = ({ theme, setTheme, onClose }: { theme: string; setTheme: (t: string) => void; onClose: () => void }) => (
  <>
    ...
    <div className="panel-body">
      ...
      <div className="panel-section">
        ...
        <div className="setting-row">
          ...
          <div className="sr-body"><div className="sr-name">Régions synodales</div><div className="sr-desc">Cercles verts des 22 régions.</div></div>
...
const DetailPanel = ({ item, onClose, saved, onToggleSave, onPick, mode, onLoginRequired }: {
  item: AnyItem | null; onClose: () => void; saved: AnyItem[];
  onToggleSave: (item: AnyItem) => void; onPick: (item: AnyItem) => void;
  mode: MapMode; onLoginRequired: () => void;
}) => {
  ...
  const handleSave = () => {
    if (mode === 'public') onLoginRequired();
    else onToggleSave(item);
  };
    ...
    <div className="detail-panel open">
      <div className="dp-hero">
        ...
        <div className="dp-hero-actions">
          <button className={isSaved ? 'starred' : ''} onClick={handleSave} title={mode === 'public' ? 'Connexion requise' : 'Favori'}>
          ...
          <button onClick={onClose}><Icon name="close" size={14} stroke={2} /></button>
        </div>
      </div>
      ...
      <div className="dp-actions">
        ...
        <button className="dp-action-btn ghost" onClick={handleSave}>
          ...
          {mode === 'public' ? 'Connexion requise' : (isSaved ? 'Enregistré' : 'Enregistrer')}
        </button>
      ...
      {tab === 'apercu' && (
        <div className="dp-section">
          {isRegion && <><h4>Vue d&apos;ensemble de la région</h4><div className="big-stat-row"><div className="big-stat green"><div className="bs-value">{regionDistricts.length}</div><div className="bs-label">Districts</div></div><div className="big-stat blue"><div className="bs-value">{regionParishes.length}</div><div className="bs-label">Paroisses</div></div></div></>}
          {isDistrict && <><h4>Vue d&apos;ensemble du district</h4><div className="big-stat-row"><div className="big-stat green"><div className="bs-value">{districtParishes.length}</div><div className="bs-label">Paroisses</div></div><div className="big-stat blue"><div className="bs-value">{region?.city}</div><div className="bs-label">Région</div></div></div></>}
          {isParish && (item as any).stats && <><h4>Statistiques 2025</h4><div className="big-stat-row"><div className="big-stat green"><div className="bs-value">{(item as any).stats.communiants}</div><div className="bs-label">Communiants</div></div><div className="big-stat orange"><div className="bs-value">{(item as any).stats.nonCommuniants}</div><div className="bs-label">Non-communiants</div></div><div className="big-stat blue"><div className="bs-value">{(item as any).stats.fideles}</div><div className="bs-label">Total</div></div></div></>}
        </div>
      )}
      {tab === 'infos' && (
        <div className="dp-section">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <InfoLine ico="clock" label={isParish ? 'Culte dominical : 09 h 00 · 11 h 00' : 'Ouvert du Lun. au Ven.'} />
      ...
      {tab === 'workers' && isParish && (
        <div className="dp-section">
          <h4>Ouvriers ecclésiastiques · {workers.length}</h4>
          {workers.map(w => {
...
/* Legend */
const Legend = ({ counts, basemap, visibleCount }: { counts: Record<string, number>; basemap: string; visibleCount: number }) => (
  <footer className="legend">
    <div className="legend-title"><span className="lt-flag" /><span className="lt-text">Légende EEC</span></div>
    <div className="legend-items">
      <span className="legend-item"><span className="l-glyph" style={{ background: '#2E9744' }}><Icon name="region" size={13} color="#fff" stroke={1.8} /></span>Région <span className="l-count">22</span></span>
      {ENTITY_TYPES.map(t => (
        <span key={t.id} className="legend-item"><span className="l-glyph" style={{ background: t.color }}><Icon name={TYPE_ICON[t.id]} size={13} color="#fff" stroke={1.9} /></span>{t.singular} <span className="l-count">{counts[t.id]}</span></span>
      ))}
    </div>
...
const ListView = ({ items, onPick }: { items: AnyItem[]; onPick: (item: AnyItem) => void }) => (
  <div className="listview">
    ...
    <div className="lv-cols"><span /><span>Nom</span><span>Type</span><span>Région · Ville</span><span style={{ textAlign: 'right' }}>Fidèles</span><span>Statut</span></div>
        ...
        return (
          <div key={it.id} className="lv-row" onClick={() => onPick(it)}>
            <div className="lv-main">
              ...
              <span className="lv-status"><span style={{ width: 8, height: 8, borderRadius: 4, background: '#2E9744', display: 'inline-block' }} />Opérationnel</span>
            </div>
          </div>
        );
      })}
...
export default function EECMapApp({ mode, user }: { mode: MapMode; user: MapUser | null }) {
  ...
  const addToRecents = useCallback((item: AnyItem) => {
    setRecents(prev => {
      ...
      return [{ ...item, _at: Date.now(), _timeAgo: "à l'instant" }, ...f].slice(0, 25);
    });
  }, []);
  ...
  const handleSearchPick = useCallback((kind: string, item: any) => {
    if (kind === 'item') { setSelected(item); addToRecents(item); setFocusTarget({ lat: item.lat, lng: item.lng, zoom: 13 }); }
    else if (kind === 'region') { setFilters(f => ({ ...f, region: item.id, district: null, parish: null })); setFocusTarget({ lat: item.lat, lng: item.lng, zoom: 8 }); }
    else if (kind === 'district') { setFilters(f => ({ ...f, region: item.regionId, district: item.id, parish: null })); setFocusTarget({ lat: item.lat, lng: item.lng, zoom: 11 }); }
  }, [addToRecents]);
  ...
  const { mapRef } = useLeafletMap('eec-map-visitor', filteredItems, basemap, selected?.id, handleMarkerClick, focusTarget);
    ...
    <div className={'app' + (fullscreen ? ' fs' : '')}>
      ...
      <div className={'main' + (activeTab ? ' panel-open' : '') + (railCollapsed ? ' rail-collapsed' : '') + (fullscreen ? ' fullscreen' : '')}>
        {!fullscreen && !railCollapsed && (
          <MapRail active={activeTab} setActive={setActiveTab} onCollapse={() => setRailCollapsed(true)}
        ...
        {!fullscreen && (
          <aside className="panel">
            ...
            {activeTab === 'parcours'  && <ParcoursPanel onClose={() => setActiveTab(null)} mode={mode} />}
            ...
            {activeTab === 'regions'   && <EntityListPanel title="Régions" items={REGIONS} kind="region" onPick={(k, i) => handleSearchPick(k, i)} onClose={() => setActiveTab(null)} totalLabel={`${REGIONS.length} régions synodales`} />}
            {activeTab === 'districts' && <EntityListPanel title="Districts" items={DISTRICTS} kind="district" onPick={(k, i) => handleSearchPick(k, i)} onClose={() => setActiveTab(null)} totalLabel={`${DISTRICTS.length} districts`} />}
            {activeTab === 'paroisses' && <EntityListPanel title="Paroisses" items={PARISHES} kind="parish" onPick={(k, i) => handleSearchPick(k, i)} onClose={() => setActiveTab(null)} totalLabel={`${PARISHES.length} paroisses`} />}
            {activeTab === 'oeuvres'   && <EntityListPanel title="Œuvres" items={OEUVRES} kind="oeuvre" onPick={(k, i) => handleSearchPick(k, i)} onClose={() => setActiveTab(null)} totalLabel={`${OEUVRES.length} œuvres`} />}
            {activeTab === 'ouvriers'  && <EntityListPanel title="Ouvriers" items={WORKERS} kind="worker" onClose={() => setActiveTab(null)} totalLabel={`${WORKERS.length} ouvriers ecclésiastiques`} />}
        ...
        <div className="map-wrap">
          ...
          <div id="eec-map-visitor" className="map" style={{ display: view === 'map' ? 'block' : 'none' }} />
          {view === 'list' && <ListView items={filteredItems} onPick={handleMarkerClick} />}

          {view === 'map' && (
            <>
              {activeTab !== 'search' && activeTab !== 'filters' && (
                <form className="floating-search" onSubmit={e => { e.preventDefault(); }} style={{ left: (fullscreen || railCollapsed) ? 60 : 56 }}>
              ...
              <div className="map-ctrls-right">
                <div className="map-ctrl-card">
                  <button onClick={recenter} title="Recentrer"><Icon name="home" size={16} stroke={1.9} /></button>
                  <button onClick={() => setBasemap(b => b === 'light' ? 'sat' : 'light')} title="Fond carte"><Icon name="layers" size={15} stroke={1.9} /></button>
                  {fullscreen
                ...
                <div className="map-ctrl-card">
                  <button className="yellow" title="Ma position (GPS)"><Icon name="pinFilled" size={14} fill="currentColor" stroke={0} /></button>
                </div>
              </div>
